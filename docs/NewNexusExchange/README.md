...
- **D3 = E3+E1:** Module guests now load through a shared loopback file-server transport. `getModuleEntry` (`src/main/moduleFiles.js`) returns the module-scoped loopback URL for both production and development modules, so development no longer depends on a `file://` entry path or missing CSP headers. E1 enforcement in `src/main/webviewSecurity.js` still keys off the unique session partition (`nexus-module:<random>`) with a deny-by-default `webRequest` filter, blackhole proxy (loopback/file-server bypass only), and requested WebRTC disabling. Both modes may load only their assigned loopback asset prefix (`getDomain()` + `/modules/<name>/`); `file:` requests are rejected. CSP on file-server responses remains defense in depth. Packaged-Electron WebRTC/STUN/TURN denial stays a manual release gate until end-to-end validated. Under E3, capabilities grant *broker methods*, not egress.
...
| Module attempts direct exfiltration (fetch, WebSocket, WebRTC/STUN, `file:` navigation) | E1: per-partition `webRequest.onBeforeRequest` deny-all allowing only the module-scoped loopback asset prefix for production and development + blackhole proxy (bypass loopback/file-server origin only) + WebRTC disabling requested on module guests (packaged peer-connection/STUN/TURN denial remains a manual release gate). E3: no capability ever grants raw egress. |
