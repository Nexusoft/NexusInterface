...
import { coreInfoAtom, coreConnectedAtom } from 'lib/coreInfo';
import { settingsAtom } from 'lib/settings';

export const coreOutputAtom = atom([]);
export const coreOutputPausedAtom = atom(false);
export function togglePaused() {
  jotaiStore.set(coreOutputPausedAtom, (paused) => !paused);
}
...
function printCoreOutput(output) {
  jotaiStore.set(coreOutputAtom, (currentOutput) => {
...
function processDeamonOutput(debugfile) {
  ...
  printCoreOutputTimer = setInterval(() => {
    const paused = jotaiStore.get(coreOutputPausedAtom);
    if (paused) {
      return;
    }
    if (batch.length == 0) {
