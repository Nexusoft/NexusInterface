...
export function startCoreOuputWatch() {
  ...
  let datadir = state.settings.coreDataDir;

  const calcDebugFileLocation = () => {
    if (fs.existsSync(path.join(datadir, subFolder, 'log', '0.log'))) {
      return path.join(datadir, subFolder, 'log', '0.log');
    } else {
      return path.join(datadir, subFolder, 'debug.log');
    }
  };

  debugFileLocation = calcDebugFileLocation();
  ...
  if (!legacyMode) {
    observeStore(
      ...
      (testnet) => {
        ...
        const logFile = calcDebugFileLocation();
        if (debugFileLocation != logFile) {
          // File has switched so unwatch then watch new file
          fs.stat(logFile, (err, stat) => {
            if (err == null) {
              debugFileLocation = logFile;
              stopCoreOuputWatch();
