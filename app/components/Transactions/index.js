...
class Transactions extends Component {
  ...
  componentDidMount() {
    ...
    window.addEventListener(
      "resize",
      this.updateChartAndTableDimensions.bind(this)
    );
      ...
      this.setState({
        ...
        zoomDomain: {
          x: [
            ...
            new Date(
              tempWalletTransactions[tempWalletTransactions.length - 1].time *
                1000
            )
          ]
        }
      ...
      Promise.all(promisList).then(payload => {
        ...
        payload.forEach(element => {
          for (let index = 0; index < element.length; index++) {
            ...
            let gothistorydata = this.findclosestdatapoint(
              tempTrans.time.toString()
            );
  ...
  getAllhourData() {
    ...
    let finishandsavefilepromise = new Promise(
 ...
 */
  ...
  ttttttt(incomingIndex) {
    if (this.wwwwww(incomingIndex)) {
      ...
      let founddata = this.findclosestdatapoint(
        this.state.walletTransactions[incomingIndex].time.toString()
      );
  ...
  addhistorydatatoprevious() {
    ...
    for (let index = 0; index < tempdata.length; index++) {
      let founddata = this.findclosestdatapoint(
        this.state.walletTransactions[index].time.toString()
      );
      if (founddata != undefined) {
  ...
  returnModalInternal() {
    ...
    if (this.state.hoveringID != 999999999999) {
      const selectedTransaction = this.state.walletTransactions[
        this.state.hoveringID
      ];
        ...
        <div className="panel">
          ...
          <div id="transactions-details">
            <Table
              ...
              data={data}
              columns={columns}
              selectCallback={this.tryingsomething}
              defaultsortingid={1}
